  ...
  zlib.inflateRaw(buffer, function (err, result) {
    ...
    try {
      ...
      if (rootElement.localName == "AuthnRequest") {
        ...
        var requestedAuthnContext = rootElement.getElementsByTagNameNS(
          SAMLP_NS,
        ...
        info.logout.response =
          ...
          'xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="_' +
          crypto.randomBytes(21).toString("hex") +
          '" Version="2.0" IssueInstant="' +
          new Date().toISOString() +
          '" Destination="' +
          info.logout.callbackUrl +
          '">' +
          "<saml:Issuer>" +
          options.issuer +
          "</saml:Issuer>" +
...
exports.createAssertion = function (options) {
  ...
  doc.documentElement.setAttribute(
    "ID",
    "_" + (options.uid || crypto.randomBytes(21).toString("hex"))
  );
  if (options.issuer) {
  ...
  var confirmationData = doc.documentElement.getElementsByTagName(
    "saml:SubjectConfirmationData"
  );

  if (options.lifetimeInSeconds) {
    var expires = new Date(
      Date.now() + options.lifetimeInSeconds * 1000
    ).toISOString();
  ...
  if (options.audiences) {
    var audienceRestrictionsElement = doc.createElementNS(
      ASSERTION_NS,
  ...
  if (options.attributes) {
    var statement = doc.createElementNS(
      ASSERTION_NS,
  ...
  var nameID = doc.documentElement.getElementsByTagNameNS(
    ASSERTION_NS,
    "NameID"
  )[0];
  ...
  if (options.authnContextClassRef) {
    var authnCtxClassRef = doc.getElementsByTagName(
...
exports.createResponse = function (options) {
  ...
  response +=
    '<samlp:Status><samlp:StatusCode Value="' + options.samlStatusCode + '"/>';
  if (options.samlStatusMessage) {
    response +=
      "<samlp:StatusMessage>" +
      options.samlStatusMessage +
      "</samlp:StatusMessage>";
  }
