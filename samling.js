...
function logout(info) {
  ...
  if (info) {
    ...
    location.href =
      ...
      delim +
      "SAMLResponse=" +
      encodeURIComponent(btoa(info.response));
  } else {
...
$(function() {
  ...
  $("#copyResponseToClipboard").click(function() {
    window.CLIPBOARDJS.copy($("#samlResponseText").val());
  ...
  $("#createResponse").click(function() {
    ...
    var response = window.SAML.createResponse({
      ...
      assertion: assertion,
      samlStatusCode: $("#samlStatusCode")
        .val()
        .trim(),
      samlStatusMessage: $("#samlStatusMessage")
        .val()
        .trim()
    });
    $("#samlResponseText").val(response);
  ...
  $("#postSAMLResponse").click(function(event) {
    ...
    if (sessionDuration !== "0") {
      expires =
        "expires=" +
        new Date(
          Date.now() + parseInt(sessionDuration) * 1000 * 60
        ).toUTCString();
    }
